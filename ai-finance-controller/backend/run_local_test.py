...
def run_test():
    ...
    try:
        ...
        print(
            f"Agent found {len(exceptions)} pairs. Calculating vectors and analyzing..."
        )
        ...
        for exc in exceptions:
            bank = (
                ...
                .filter(BankTransaction.transaction_id == exc.source_record_id)
                .first()
            )
            ledger = (
                db.query(LedgerEntry)
                .filter(LedgerEntry.ledger_id == exc.candidate_record_id)
                .first()
            )
        ...
        audit_results = verifier.verify_and_commit_ai_matches(
            run_id, final_state["resolved_matches"]
        )
